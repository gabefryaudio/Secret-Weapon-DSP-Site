...
class UpperCompGUI extends HTMLElement {
  ...
  animate() {
    ...
    // For GR (negative values), we typically just use current
    ...
    Object.keys(meterMap).forEach(param => {
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (cfg.ascending) {
          ...
          if (value >= dotDb) {
            intensity = 1.0;
          } else if (value < dotDb - dbStep) {
            intensity = 0.0;
          } else {
            intensity = (value - (dotDb - dbStep)) / dbStep;
          }
        } else {
          // For gain reduction
          const clampVal = Math.min(0, value);
          if (clampVal >= -0.01) {
            intensity = 0.0;
  ...
  getHTML() {
    ...
    return `
      ...
      #compressor {
        ...
        max-width: 1200px;   /* won't exceed 1200px wide */
      ...
      .sections-container {
        display: flex;
        gap: 16px;
        /* Use full height inside #compressor. */
        width: 100%;
        height: 100%;
      }
