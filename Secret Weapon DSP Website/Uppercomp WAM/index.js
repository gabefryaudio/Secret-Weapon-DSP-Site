...
<head>
  ...
  <link
    ...
    integrity="sha512-lG9C+Qh7hCXS6ETqE9Xd9Q5H8u0lk//Sezyw8W13ty2Z85CPG7cmWOTKMIm//lm4Yz8WQkX8xkGpC8N8bHIpGA=="
...
<body>
  ...
  <!-- =====================
       ...
       ===================== -->
  <!-- Remove individual script tags for WAM files -->
  <!-- Instead, use a module to import and initialize your worklet -->
  <script type="module">
    // Import the patch connection creator from your WAM module.
    import { createAudioWorkletNodePatchConnection } from "./Uppercomp WAM/cmaj_Uppercomp.js";

    let audioCtx;
    let compressorConnection;
    ...
    // Helper: play a sample through the compressor
    async function playSample(url) {
      await audioCtx.resume();
      const audioBuffer = await loadSample(url);
      const source = audioCtx.createBufferSource();
      source.buffer = audioBuffer;
      // Connect source -> Uppercomp patch -> destination
      source.connect(compressorConnection.node);
      source.start();
    }
    ...
    window.addEventListener('DOMContentLoaded', async () => {
      ...
      try {
        // Initialise the Cmajor patch via the exported function.
        compressorConnection = await createAudioWorkletNodePatchConnection(audioCtx, 'uppercomp-worklet');
        // Connect the patch's output to the context destination.
        compressorConnection.node.connect(audioCtx.destination);
        console.log("Uppercomp worklet loaded successfully.");
      } catch (err) {
        console.error("Error loading Uppercomp worklet:", err);
      }

      // Attach event listeners to track buttons to load different samples.
      ...
      trackButtons.forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          // Adjust these paths as needed based on your repository structure and file names.
          if (text === 'Drums') {
            playSample('./Audio Files/Drums.wav');
          } else if (text === 'Bass') {
            playSample('./Audio Files/Bass.wav');
          } else if (text === 'Vocals') {
            playSample('./Audio Files/Vocals.wav');
          } else if (text === 'Guitar') {
            playSample('./Audio Files/Guitar.wav');
          } else if (text === 'Full Mix') {
            playSample('./Audio Files/FullMix.wav');
          }
        });
      });

      // Optionally, attach an event listener to the play button.
